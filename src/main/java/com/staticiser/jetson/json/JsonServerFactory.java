...
public class JsonServerFactory<Service> extends ServerFactory<Service> {
    ...
    public JsonServerFactory(final Class<Service> service_type, final JsonFactory json_factory) {
        super(service_type, new ServerChannelInitializer(new JsonRequestDecoder(json_factory, ReflectionUtil.mapNamesToMethods(service_type)), new JsonResponseEncoder(json_factory)));
    }
}
